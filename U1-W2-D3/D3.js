...
switch (element.eye_color){
  ...
  eyeColor.red.push(element);
    break;
    case "blue-gray": 
    eyeColor["blue-gray"].push(element);
      break;
      default:
        console.log("colore occhi non ricnosciuto");
  }
}
...
while (index < starWarsCharacters.length){
  ...
  crewMass = crewMass + element.mass;
  index++;
}
...
for (let i = 0; i < starWarsCharacters.length; i++){
  ...
  const elementFemaleCharacter = femaleCharacters[j];

if (elementFemaleCharacter.name === element) {
